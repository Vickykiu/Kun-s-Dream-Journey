...
@export var memory_1_shake_enabled: bool = true
@export var memory_2_shake_enabled: bool = false

@export var memory_shake_duration: float = 0.35
@export var memory_shake_strength: float = 12.0
@export var memory_shake_interval: float = 0.03
...
func start_memory_sequence() -> void:
	...
	# Shake Memory 1 if enabled.
	if memory_1_shake_enabled:
		shake_memory_image()
	...
	# Shake Memory 2 if enabled.
	if memory_2_shake_enabled:
		shake_memory_image()
...
func show_memory_image(
	...
	# Reset image position before showing.
	memory_texture_rect.position = (
		Vector2.ZERO
	)
...
func shake_memory_image() -> void:
	...
	while elapsed < memory_shake_duration:
		var remaining: float = (
			1.0
			- elapsed / memory_shake_duration
		)

		var current_strength: float = (
			memory_shake_strength
			* remaining
		)
		...
		await get_tree().create_timer(
			memory_shake_interval
		).timeout

		elapsed += memory_shake_interval
