...
func _ready() -> void:
	...
	sfx_slider.set_value_no_signal(MusicManager.get_sfx_volume())
	_update_percentage_labels()

	master_slider.value_changed.connect(_on_master_volume_changed)
	music_slider.value_changed.connect(_on_music_volume_changed)
	sfx_slider.value_changed.connect(_on_sfx_volume_changed)
	%TestSFXButton.pressed.connect(_on_test_sfx_pressed)
...
func _on_sfx_volume_changed(value: float) -> void:
	...
	_update_percentage_labels()
	MusicManager.play_sfx(&"ui_confirm")


func _on_test_sfx_pressed() -> void:
	MusicManager.play_sfx(&"perfect")
